...
async fn test_handler() {
    ...
    let mut service = ServiceBuilder::new()
        .layer(ProtoHttp1ServerMakeLayer::new(ProtoHttp1ServerConfig {
            max_header_size: 0,
            max_body_size: 0,
...
{
    type Response = Http1ServerResponseEvent;
    type Error = ();
                ...
                HTTP1ServerEvent::ProtocolUpgrade(_req, _resp, (mut read, mut write)) => {
                    ...
                    match read.read(&mut buf).await {
                        Ok(n) => match write.write_all(&buf[..n]).await {
                            Ok(_n) => Ok(Http1ServerResponseEvent::NoResponseExpected),
                            Err(_error) => Ok(Http1ServerResponseEvent::NoResponseExpected),
                        },
                        Err(_) => Ok(Http1ServerResponseEvent::NoResponseExpected),
                    }
                }
            }
        })
