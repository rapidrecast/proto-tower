...
impl<READER, WRITER, SERVICE, ERROR> Service<(READER, WRITER)> for ProtoHttp1Layer<SERVICE>
where
    READER: AsyncReadExt + Send + Unpin + 'static,
    WRITER: AsyncWriteExt + Send + Unpin + 'static,
    SERVICE:
        Service<HTTP1Request, Response = HTTP1Response, Error = ERROR> + Send + Clone + 'static,
{
    ...
    fn call(&mut self, (mut reader, mut writer): (READER, WRITER)) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            // Send response
            writer
                .write_all("This is a response".as_bytes())
                .await
                .unwrap();
            Ok(())
        })
